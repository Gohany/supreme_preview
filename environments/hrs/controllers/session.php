...
class session extends controller
{
	...
	public function create()
	{
		...
		//Create session
		$this->{self::PRIMARY_MODEL}->sessions()->add($this->{self::PRIMARY_MODEL}->srp->serverSessionKey, $this->input['type']);
		...
		if (!$this->{self::PRIMARY_MODEL}->write('hrsLogins', array(
				'action' => 'insert',
				'data' => $login,
			)))
		{
		...
		$clientInfo = new clientInfo(clientInfo::CLIENTTYPE_CLIENT, $this->{self::PRIMARY_MODEL}->user_id, 'base', $this->{self::PRIMARY_MODEL}->srp->serverSessionKey);
